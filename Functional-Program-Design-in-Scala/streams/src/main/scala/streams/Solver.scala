...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    def loop(neighbors: List[(Block, Move)]): LazyList[(Block, List[Move])] = neighbors match {
      case Nil => LazyList.empty
      case (block, move) :: ns => (block, move :: history) #:: loop(ns)
    }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, history) #:: tn =>
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(block, history), explored)
      newNeighbors #::: from(tn #::: newNeighbors, explored ++ newNeighbors.map(_._1))
  }
