...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction(new PartialFunction[Json, Int] {
      override def isDefinedAt(x: Json): Boolean = x match {
        case Json.Num(value) => value.isValidInt
        case _ => false
      }
      override def apply(v1: Json): Int = v1 match
      { case n: Json.Num => n.value.intValue }
    })
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case as: Json.Arr =>
        val decoded = as.items.flatMap(decoder.decode)
        if (decoded.size == as.items.size) Option(decoded) else Option.empty
    }
  ...
  /**
    ...
    */
  implicit def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case obj: Json.Obj => obj.fields.get(name).flatMap(decoder.decode)
    }
}
