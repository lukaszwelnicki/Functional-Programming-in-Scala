...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(c => c).map { case (ch, str) => (ch, str.length) }.toList
    .sortBy(pair => pair._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x.map { case (char, count) => (char, count - yMap.getOrElse(char, 0))}.filter { case (_, count) => count > 0 }
  }
